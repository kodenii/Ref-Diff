...
def inference(test_loader, model, args):
    ...
    for img, param in tbar:
        ...
        ori_img = Image.fromarray(param['ori_img'][0].cpu().numpy(), mode="RGB")
        # dump image & mask
        if args.visualize:
        ...
        for sent in param['sents']:
            mask = mask / 255.
            file_name = param['img_name']
            text = tokenize(sent, args.word_len, True)
            text = text.cuda(non_blocking=True)
            # inference
            pred = model(img, text, ori_img, sent, file_name)
    ...
    for thres in torch.arange(0.5, 1.0, 0.1):
        ...
        prec_list.append(tmp)
    iou = iou_list.mean()
    miou = np.average(iou_list.cpu(), weights=size_list)
    prec = {}
    for i, thres in enumerate(range(5, 10)):
        key = 'Pr@{}'.format(thres*10)
        value = prec_list[i].item()
    ...
    logger.info('mIoU={:.2f}'.format(100.*miou.item()))
