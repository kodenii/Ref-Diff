...
class CLIP(nn.Module):
    ...
    def forward2(self, image, text):
        ...
        image_features = [image_feature / (image_feature.norm(dim=-1,
                                                              keepdim=True) + 1e-6) for image_feature in image_features]
        text_features = [text_feature / (text_feature.norm(dim=-1,
                                                           keepdim=True) + 1e-6) for text_feature in text_features]

        return image_features, text_features[0], text_features[1]
